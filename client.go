...
func (c *PingClient) Run(addr string) error {
	...
	for range ticker.C {
		...
		pong, err := c.Ping(addr)
		if err != nil {
			return err
		}

		delta := time.Since(pong.Sent.Parse())
		Output("ping %d/%d to %s took %s", pong.Sseq, pong.Rseq, addr, delta)
	}
...
func (c *PingClient) PingTLS(addr string) (*pb.Pong, error) {
...
func (c *PingClient) PingInsecure(addr string) (*pb.Pong, error) {
